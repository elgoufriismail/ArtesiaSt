...
| **Service card** | title top, description bottom, small "read more" row with only its dot visible | title block **moves up 60 px**, "read more" row **moves down 60 px** (to the card bottom) and its label fades **0 → 1**; image unchanged | ≈600 ms ease-out (settles by ~650 ms); reverts |
